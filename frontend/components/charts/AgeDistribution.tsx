...
interface AgeRecord { name: string; party: string; age: number }
...
export function AgeDistribution({ data, parties }: Props) {
  ...
  useEffect(() => {
    ...
    g.append('g')
      ...
      .call(ax => ax.selectAll('text').style('font-size', '11px').attr('dx', '-6'))
    ...
    const partyData = parties.map(party => {
      ...
      const densityToPixel = d3.scaleLinear()
        .domain([0, maxDensity])
        .range([0, violinH * 0.88])
      return { party, records, density, bandY, baseline, dotCenterY, jitterAmt, color, densityToPixel }
    })
    ...
    partyData.forEach(({ party, records, density, baseline, dotCenterY, jitterAmt, color, densityToPixel }) => {
      ...
      dataG.selectAll<SVGCircleElement, AgeRecord>(`.dot-${party.replace(/\W/g, '_')}`)
        .data(records)
        .join('circle')
        .attr('class', `dot dot-${party.replace(/\W/g, '_')}`)
        .attr('cy', (_, i) => dotCenterY + Math.sin(i * 47.431 + baseline) * jitterAmt)
        .attr('r', 2.5)
        .attr('fill', color)
        .attr('opacity', 0.6)
    })
    ...
    function update(xS: d3.ScaleLinear<number, number>) {
      ...
      gridG.call((g2: d3.Selection<SVGGElement, unknown, null, undefined>) =>
        g2.call(d3.axisBottom(xS).ticks(8).tickSize(iH).tickFormat(() => ''))
          .call(ax => ax.select('.domain').remove())
      ...
      dataG.selectAll<SVGLineElement, unknown>('.sep-line')
        .attr('x2', iW)
    }
    ...
    g.append('rect')
      ...
      .on('mousemove', (event: MouseEvent) => {
        ...
        if (!hoveredParty) { tooltip.style('opacity', '0'); return }

        const pd = partyData.find(p => p.party === hoveredParty)
        if (!pd || pd.records.length === 0) { tooltip.style('opacity', '0'); return }

        // Nearest politician by age
        const nearest = pd.records.reduce((best, d) =>
          Math.abs(d.age - ageAtMouse) < Math.abs(best.age - ageAtMouse) ? d : best
        )

        // Hide tooltip if mouse is more than 20px away from the nearest dot
        if (Math.abs(currentXS(nearest.age) - mx) > 20) { tooltip.style('opacity', '0'); return }

        tooltip
          .style('opacity', '1')
          .style('left', `${event.clientX + 12}px`)
          .style('top', `${event.clientY - 28}px`)
          .html(`<b>${nearest.name}</b><br/>${nearest.party} · ${nearest.age} Jahre`)
      })
